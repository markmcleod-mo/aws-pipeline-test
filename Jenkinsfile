...
def assumeRole(Account='005402609678', Region='eu-west-2', Creds='test-master', Role='cloud-team-admin') {
  withAWS(region:"${Region}",credentials:"${Creds}") {
    //script {
      accounts = sh (script: 'aws sts assume-role --role-arn arn:aws:iam::005402609678:role/"${Role}" --role-session-name cloud-team-admin --output text', returnStdout: true).split()
